...
extern HCURSOR CS_Cursor; //鼠标指针地址 
HINSTANCE hInstance;//应用程序实例句柄
double PIXELPERMMX;
double PIXELPERMMY;//毫米转像素 

HWND consoleWindow, graphicsWindow;
//static HWND consoleWindow, graphicsWindow;
HDC gdc, osdc; //去除static 
HBITMAP osBits; //去除static 
...
double windowWidth = DesiredWidth;
double windowHeight = DesiredHeight;
...
int RectWidth(RECT *rp);//去掉static 
int RectHeight(RECT *rp);
static void SetRectFromSize(RECT *rp, int x, int y, int width, int height);
static double Radians(double degrees);
static int Round(double x);
double InchesX(int x);//去除static 
double InchesY(int y);
int PixelsX(double x);//去除static 
int PixelsY(double y);
int ScaleX(double x);//去除static 
int ScaleY(double y);
...
static void InitDisplay(void)
{
    ...
    PIXELPERMMX = 100.0*25.4/xResolution;
    PIXELPERMMY = 100.0*25.4/yResolution; //百分之一毫米转换 
    printf("PIXELPERMMX: %f\nPIXELPERMMY: %f\n",PIXELPERMMX,PIXELPERMMY);
    initialized = TRUE;
    ...
    //SetRectFromSize(&graphicsRect, LeftMargin, TopMargin, PixelsX(windowWidth), PixelsY(windowHeight));
    SetRectFromSize(&graphicsRect, 0, 0, PixelsX(windowWidth), PixelsY(windowHeight));
    //style = WS_OVERLAPPEDWINDOW & ~(WS_MINIMIZEBOX | WS_MAXIMIZEBOX);
    style = WS_OVERLAPPEDWINDOW & ~(WS_MAXIMIZEBOX | WS_THICKFRAME);//保留了最小化 去除最大化 去除可调边距 
    
    g_keyboard = NULL;
	g_mouse = NULL;
	g_timer = NULL;
    
    wndcls.cbSize = sizeof(WNDCLASSEX);//新增 
    wndcls.cbClsExtra = 0;
    wndcls.cbWndExtra = 0;
    wndcls.hbrBackground = (HBRUSH)GetStockObject(WHITE_BRUSH);
    wndcls.hCursor = LoadCursorFromFile(FILE_CURSOR);//LoadCursor(NULL, IDC_ARROW);//光标 原本：IDC_ARROW 调用资源函数：MAKEINTRESOURCE(XXXX) 
    wndcls.hIcon = (HICON)LoadImage(NULL,FILE_ICON,IMAGE_ICON,0,0,LR_LOADFROMFILE);//LoadIcon(NULL, IDI_APPLICATION);//图标 
    wndcls.hIconSm = (HICON)LoadImage(NULL,FILE_TITLE_ICON,IMAGE_ICON,0,0,LR_LOADFROMFILE);//标题栏图标 
    wndcls.hInstance = NULL;
    wndcls.lpfnWndProc = (WNDPROC)GraphicsEventProc;
    wndcls.lpszClassName = GWClassName;
	...
	int WIDTH=(PixelsX(screenWidth)-RectWidth(&graphicsRect))/2;
	int HEIGHT=(PixelsX(screenHeight)-RectHeight(&graphicsRect)+POSCORRECT)/2;//窗口初始位置位于屏幕中央 
	printf("Original Point [%02d, %02d]\n",WIDTH,HEIGHT);
    printf("screenWidth: %f\nscreenHeight: %f\n",screenWidth,screenHeight);
    
    graphicsWindow = CreateWindowEx( // 改为Ex扩展 
      0,
      GWClassName,
      windowTitle, 
      style,
    ...
    printf("Window: %06X\n",graphicsWindow);
    SetWindowLong(graphicsWindow, GWL_STYLE, GetWindowLong(graphicsWindow, GWL_STYLE) & ~WS_CAPTION); // 去掉标题栏 √ 
    SetWindowPos(graphicsWindow, HWND_TOP, WIDTH, HEIGHT, 0, 0, SWP_NOSIZE|SWP_DRAWFRAME);
    ...
    printf("Change Window Size [%02d, %02d]\n",dx,dy);
    SetWindowPos(graphicsWindow, HWND_TOP, 0, 0, RectWidth(&graphicsRect) + dx, RectHeight(&graphicsRect) + dy, SWP_NOMOVE|SWP_DRAWFRAME);
...
static void InitDrawingTools(void)
{
    ...
    nFonts = 0;
    previousColor = 0;
    drawColor = RGB(255, 255, 255); //为了画白线hhhh 
                                         ...
                                         WPARAM wParam, LPARAM lParam)
{
    switch(msg)
    {
		...
		/*case WM_COMMAND:// 响应菜单命令:当用户单击某个菜单项后，窗口处理程序（WindowProc）会收到一条WM_COMMAND消息
			MenuMessage(wParam,lParam);
			return 0;*/
...
*/
    ...
    if (eraseMode) {
        ...
        DeleteObject(drawPen);
        LOGBRUSH lb;
		lb.lbStyle = BS_SOLID; //类型 
		lb.lbColor = RGB(255,255,255); //颜色 
		lb.lbHatch = 0; //阴影 
        drawPen = (HPEN) ExtCreatePen(PS_GEOMETRIC | PS_DASHDOT | PS_ENDCAP_ROUND, 3 , &lb, 0, NULL); //剧情需要hhh 
		//drawPen = (HPEN) CreatePen(PS_SOLID, penSize, drawColor); 
...
int RectWidth(RECT *rp)// 去掉了static 
{
